...
#Option 1: with specified "single_vs_all_markers"
markerGenes <- read_xlsx("CellTypeMetaData&MarkerGenes.xlsx")

#make every element of list into character vector of genes 
list <- list()
for(i in c(1:length(markerGenes$single_markers_vs_all))){
  genes <- unlist(strsplit(markerGenes$single_markers_vs_all[[i]], ", "))
  list[[i]] <- genes
  list
}
names(list) <- markerGenes$cluster #set cell names 

sum(sapply(list, function(x){sum(sapply(x, function(y){y == "NA"}))})) #0 empty cells (NA)
sum(sapply(sapply(list, unique), length)) #302 unique marker genes
...
sum(sapply(entrezlist, length)) # 256 markers with matched entrezID
summary(entrezlist)

# Check for duplicate markers in different cell types
all_markers <- unlist(entrezlist)
dup <- all_markers[duplicated(all_markers)] #0 duplicate markers

#inspect number of marker genes for each cell type
table(sapply(entrezlist, length))

#only use cell types with at least 4 markers
entrezlist[sapply(entrezlist, length) < 4] # these celltypes will be dropped
markerlist4 <- entrezlist[sapply(entrezlist, length) >= 4] 

#only all cell types with at least 1 marker
markerlist <- entrezlist[sapply(entrezlist, length) >= 1] 

###################################################################################
#Option 2: with all single markers concatenated together (single_vs: all_markers|level1|level2|level3|level4)
markerGenes <- read_xlsx("CellTypeMetaData&MarkerGenes.xlsx")
...
###################################################################################
#Option 3: with all combinatorial markers concatenated together (combo_markers_vs: all|level1|level2|level3|level4)
markerGenes <- read_xlsx("CellTypeMetaData&MarkerGenes.xlsx")

list <- list()
for(i in c(1:nrow(markerGenes))){
  #concatenate all cells containing marker gene
  genes <- markerGenes$combo_markers_vs_all[i]
  genes <- append(genes, markerGenes$combo_markers_vs_level1[i])
  genes <- append(genes, markerGenes$combo_markers_vs_level2[i])
  genes <- append(genes, markerGenes$combo_markers_vs_level3[i])
  vectorGenes <- character()
  for(j in c(1:length(genes))){
    #concatenate all genes from concatenated cells into one character vector
    vectorGenes <- append(vectorGenes, unlist(strsplit(genes[[j]], " | ")))
...
sum(sapply(list, function(x){sum(sapply(x, function(y){y == "NA"}))})) #12 empty cells (NA)

#convert gene symbol to entrezID
entrezlist <- list 
for(i in c(1:length(entrezlist))){
  #replace gene symbols by entrez ID
  entrezid <- mapIds(org.Hs.eg.db, entrezlist[[i]], "ENTREZID", "SYMBOL")
  for(j in c(1:length(entrezid))){
    #if org.Hs.eg.db could not match entrezID, try matching by using EnsDb.Hsapiens.v79 instead
    if(is.na(entrezid[j])){
      entrezid[j] <-  mapIds(EnsDb.Hsapiens.v79, entrezlist[[i]][j], "ENTREZID", "SYMBOL")
    }
  }
  names(entrezid) <- NULL
  entrezlist[[i]] <- unique(na.omit(entrezid)) #remove NA's and duplicate marker genes
...
sum(sapply(entrezlist, length)) # 345 markers with matched entrezID 
summary(entrezlist)

# Check for duplicate markers in different cell types
all_markers <- unlist(entrezlist)
dup <- all_markers[duplicated(all_markers)] #53 duplicate markers

#remove duplicate markers from list
entrezlist <- sapply(entrezlist, function(x) {x <- x[!x %in% dup]} )
sum(sapply(entrezlist, length)) #245 marker genes that are unique in a cell type

#inspect number of marker genes for each cell type
table(sapply(entrezlist, length))

#only use cell types with at least 2 markers
entrezlist[sapply(entrezlist, length) < 2] # these celltypes will be dropped
markerlist2 <- entrezlist[sapply(entrezlist, length) >= 2] # 68 cell types retained

#only all cell types with at least 1 marker
markerlist <- entrezlist[sapply(entrezlist, length) >= 1] # 74 cell types retained
