...
#within-sample normalization was done before additional between-donor correction.
#three normalization strategies for these 1285 left hemisphere samples (after within-sample normalization) are included here:
#1) between-donor normalization using the SRS method (done separately in matlab, here the output file is used)
#2) between-donor normalization using the Limma batch correction function
#3) between-donor normalization using the Limma batch correction function, followed by additional between-donor SRS normalization.
...
############################################################################################################
###SRS NORMALIZE GENES - OUTPUT FROM MATLAB FILE ###########################################################
############################################################################################################
...
##read in gene expressions for each sample (SRS normalized in this case)
expression <- as.data.frame(matlab.output[[6]]) #gene expressions of 10015 probes
probe_ids <- as.numeric(matlab.output[[2]][[4]]) #vector of all probe ID's
colnames(expression) <- c("parcellation", probe_ids)
...
################################################################################################################################
###SRS NORMALIZE GENES - DO SRS NORMALIZATION AND COMPARE TO OUTPUT FROM MATLAB FILE WITH options.withinSamleNormalization=TRUE#
################################################################################################################################
exprSRS <- list()
for(i in 1:length(donorNames)){
  exprSRS[[i]] <- log2WithinNorm.transposed[log2WithinNorm.transposed[,1] == donorNames[i],2:ncol(log2WithinNorm.transposed)]
}
names(exprSRS) <- donorNames

exprSRSlist <- mclapply(exprSRS, mc.cores = 4, function(x){
...
for(i in 1:length(exprSRS)){
  #replace limmaNormalized values in exprLimmaSRS with additionally SRS normalized values
  exprSRS[[i]][,2:ncol(exprSRS[[i]])] <- exprSRSlist[[i]]
} 

exprSRS <- do.call(rbind, exprSRS) #combine all donors into 1 dataframe
exprSRS<- exprSRS %>% 
...
exprSRS <- exprSRS %>% select(-"1054004") # delete probe with NaN values
#saveRDS(exprSRS, "/Users/philippehabets/Dropbox/Endo/fMRI.transcriptomics/data/Bristol_study_ultradian_rhythm/Output/withinSampleNormalization/exprSRS.RDS")
...
##samples, for each donor individually. Some genes might return NaN because of SRS on SRS-normalized values.
...
############################################################################################################
###PLOT TSNE RESULTS FOR EACH NORMALIZATION#################################################################
############################################################################################################
#unnormalized genes
...
#SRS normalized genes (after within-sample normalization)
tsne <- Rtsne(exprSRS[,3:ncol(exprSRS)], perplexity = 30, max_iter = 1000, theta = 0.0, pca = FALSE, normalize = TRUE, verbose = TRUE) ##pca_scale (& -center) = TRUE if pca = TRUE
df_tsne <- as.data.frame(tsne$Y)
df_tsne <- cbind(donor = exprSRS$donor, df_tsne)
ggplot(df_tsne, aes(x = V1, y = V2, color = donor)) +
  geom_point() +
  xlab("tSNE_1") +
  ylab("tSNE_2") +
