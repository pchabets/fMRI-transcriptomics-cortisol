...
#######optional: background list of all 10014 RNAseq correlated probes taken into account for analysis,  where each probe = one gene###### 
# path_probeInfo <- file.choose() #path to probeInfo file
# path_normalizedProbes<- file.choose() #path to output file with Limma+SRS normalized gene expressions (exprLimmaSRS.RDS where within sample normalisation = TRUE)
# 
# probeInfo <- read.csv2(path_probeInfo)
# probeInfo$probe_id <- as.character(probeInfo$probe_id)
# normalizedProbes <- readRDS(path_normalizedProbes)
# backgroundProbes <- colnames(normalizedProbes[,-c(1:2)])
# background <- data.frame("probe_id" = backgroundProbes)
# background <- inner_join(background, probeInfo) %>%
#   dplyr::select("gene_symbol", "gene_name", "entrez_id", "probe_name", "probe_id")
# background_entrezIDs <- as.character(background$entrez_id)
# 
# BackgroundGenes <- addList(david, inputIds = background_entrezIDs, idType = "ENTREZ_GENE_ID", listName = "background", listType = "Background")
# getBackgroundListNames(david)
#########################################################
...
# ######repeat for all together##############
# Specifiy annotation categories (KEGG)
# setAnnotationCategories(david, c("KEGG_PATHWAY", "GOTERM_BP_ALL", "GOTERM_MF_ALL", "GOTERM_CC_ALL"))
# 
# # Get functional annotation chart as R object.
# FuncAnnotChart <- getFunctionalAnnotationChart(david)
# 
# # Print functional annotation chart to file.
# getFunctionalAnnotationChartFile(david, "/Users/philippehabets/Dropbox/Endo/fMRI.transcriptomics/data/Bristol_study_ultradian_rhythm/Output/RDAVID.wilcoxon.test/FuncAnnot_CHART_ALL_noBackground.csv")
# 
# # Get functional annotation clustering
# FuncAnnotClust <- getClusterReport(david)
# 
# # Print functional annotation clustering to file (limited to 3000 genes).
# getClusterReportFile(david, "/Users/philippehabets/Dropbox/Endo/fMRI.transcriptomics/data/Bristol_study_ultradian_rhythm/Output/RDAVID.wilcoxon.test/FuncAnnot_CLUSTER_ALL_noBackground.csv")
# 
# # Get functional table (for each gene the KEGG categories)
# getFunctionalAnnotationTableFile(david, "/Users/philippehabets/Dropbox/Endo/fMRI.transcriptomics/data/Bristol_study_ultradian_rhythm/Output/RDAVID.wilcoxon.test/FuncAnnot_TABLE_ALL_noBackground.csv")
# 
